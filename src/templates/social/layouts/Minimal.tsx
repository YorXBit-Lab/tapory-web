...
export function SocMinimal({ data, c }: LayoutProps) {
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center px-8 py-8"
      ...
      <p className="mb-6 text-[9px] font-bold tracking-[0.5em] uppercase" style={{ color: c.secondary }}>PROFILE</p>
      ...
      <p className="mt-1 text-center text-[10.5px]" style={{ color: c.secondary }}>
        {data.subtitle || 'Bio / Tagline'}
      </p>

      <div className="my-5 flex w-full items-center gap-3">
        <div className="h-px flex-1 opacity-10" style={{ backgroundColor: c.primary }} />
        <span className="text-[11px]" style={{ color: c.secondary }}>✦</span>
        <div className="h-px flex-1 opacity-10" style={{ backgroundColor: c.primary }} />
      </div>
      ...
      {data.description && (
        <>
          ...
          <p className="text-center text-[10px] leading-[1.8] opacity-45" style={{ color: c.primary }}>
            {data.description}
          </p>
        </>
      )}

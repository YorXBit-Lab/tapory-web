    ...
    return (
      <div className="relative flex min-h-full w-full flex-col items-center justify-center gap-4 px-6"
        ...
        <div className="relative z-10 flex flex-col items-center gap-1 text-center">
          <p className="text-[11px] font-bold uppercase tracking-[0.3em] opacity-35" style={{ color: c.primary }}>
            Chuyển hướng
          </p>
          <p className="mt-1 text-[9.5px] leading-relaxed opacity-40" style={{ color: c.primary }}>
    ...
    <div className="relative flex h-full w-full flex-col bg-white">
      ...
      <div className="flex flex-shrink-0 items-center gap-1.5 px-2 py-1.5"
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1 rounded px-2 py-0.5"
          ...
          <span className="truncate" style={{ fontSize: 9, color: '#202124' }}>{hostname}</span>
        </div>
        <a href={url} target="_blank" rel="noopener noreferrer"
          className="flex h-[18px] w-[18px] flex-shrink-0 items-center justify-center rounded"
          style={{ background: '#e8f0fe', textDecoration: 'none', fontSize: 11, color: '#1a73e8' }}>↗</a>
      </div>
      ...
      {loading && !error && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-2 bg-white"
          ...
          <p style={{ fontSize: 10, color: c.primary, opacity: .6 }}>Đang tải {hostname}…</p>
        </div>
      )}
      ...
      {error && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 px-6"
          ...
          <p className="text-center text-[13px] font-bold" style={{ color: c.primary }}>{hostname}</p>
          <p className="max-w-[180px] truncate text-center text-[9px] opacity-50" style={{ color: c.primary }}>{url}</p>
          <a href={url} target="_blank" rel="noopener noreferrer"
            className="mt-2 flex items-center gap-1.5 rounded-xl px-5 py-2 text-[10px] font-bold"

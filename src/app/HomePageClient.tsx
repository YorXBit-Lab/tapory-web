...
const CONTACT_LINKS = [
  { key: 'email', label: 'Email', href: 'mailto:yorxbit@gmail.com', color: '#8B6B52' },
  { key: 'tiktok-shop', label: 'TikTok Shop', href: 'https://tiktok.com/shop', color: '#010101' },
];
const ORDER_URL = 'https://tiktok.com/shop';
...
/* ══════════════════════════════════════════
   PAGE
══════════════════════════════════════════ */
export default function HomePageClient({ initialProducts = [] }: { initialProducts?: IProduct[] }) {
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const { data, isLoading } = useProducts();
  const products = data ?? initialProducts;
  const productsLoading = isLoading && products.length === 0;
  return (
